<div class="row">
    ...
    <script>
        ...
        var obtenerCarrito = function () {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var result = jQuery.parseJSON(data);
                    dibujarResumenCarro(result.carro, result.envio, result.total);
                    $('#divCarrito').css("margin-top", $('#plb')[0].offsetTop).css("display", "block");
                }
            });
        };
        var aux = null;
        ...
        var dibujarResumenCarro = function (items, envio, total) {
            ...
            if (envio == 0 || total == 0) {
                $('#rowEnvio').css('display', 'none');
            } else {
                $('#rowEnvio').css('display', '');
                $('.cenvio').text('$' + envio);
            }
        ...
        $(window).on("scroll", function () {
            ...
            if (st > lastScrollTop) {
                ...
                if (st < headerHeight + carritoTop) {
                    if (st > headerHeight) {
                        $("#divCarrito").css("margin-top", st-headerHeight+30);
                    } else {
                        $("#divCarrito").css("margin-top", 30);
                    }
                }
